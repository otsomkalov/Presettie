...
type SetPresetSizeMessageHandler() =
  ...
  do
    resourceProvider.Setup(_.Item(Messages.SendPresetSize)).Returns(Messages.SendPresetSize)
    |> ignore

  do
    resourceProvider.Setup(_.Item(Messages.PresetSizeTooSmall)).Returns(Messages.PresetSizeTooSmall)
    |> ignore

  do
    resourceProvider.Setup(_.Item(Messages.PresetSizeTooBig)).Returns(Messages.PresetSizeTooBig)
    |> ignore

  do
    resourceProvider.Setup(_.Item(Messages.PresetSizeNotANumber)).Returns(Messages.PresetSizeNotANumber)
    |> ignore
...
type CreatePresetButtonMessageHandler() =
  ...
  do
    resourceProvider.Setup(_.Item(Messages.SendPresetName)).Returns(Messages.SendPresetName)
    |> ignore

  do chatCtx.Setup(_.AskForReply(Messages.SendPresetName)).ReturnsAsync(()) |> ignore
...
type IncludePlaylistButtonMessageHandler() =
  ...
  do
    resourceProvider.Setup(_.Item(Messages.SendIncludedPlaylist)).Returns(Messages.SendIncludedPlaylist)
    |> ignore
...
type ExcludePlaylistButtonMessageHandler() =
  ...
  do
    resourceProvider.Setup(_.Item(Messages.SendExcludedPlaylist)).Returns(Messages.SendExcludedPlaylist)
    |> ignore
...
type ExcludeArtistButtonMessageHandler() =
  let musicPlatform = Mock<IMusicPlatform>()
  let musicPlatformFactory = Mock<IMusicPlatformFactory>()
  ...
  do
    resourceProvider.Setup(_.Item(Buttons.ExcludeArtist)).Returns(Buttons.ExcludeArtist)
    |> ignore

  do
    resourceProvider.Setup(_.Item(Messages.SendExcludedArtist)).Returns(Messages.SendExcludedArtist)
    |> ignore

  let handler =
    Message.excludeArtistButtonMessageHandler musicPlatformFactory.Object authService.Object resourceProvider.Object chatCtx.Object

  [<Fact>]
  member _.``should ask for excluded artist when platform present``() =
    musicPlatformFactory.Setup(_.GetMusicPlatform(Mocks.userId.ToMusicPlatformId())).ReturnsAsync(Some musicPlatform.Object)
    chatCtx.Setup(_.AskForReply(Messages.SendExcludedArtist)).ReturnsAsync(())
...
type TargetPlaylistButtonMessageHandler() =
  ...
  do
    resourceProvider.Setup(_.Item(Messages.SendTargetedPlaylist)).Returns(Messages.SendTargetedPlaylist)
    |> ignore
...
type ExcludePlaylistMessageHandler() =
  ...
  member _.``should handle reply with excluded playlist and send message``() =
    ...
    presetService.Setup(_.ExcludePlaylist(It.IsAny<ExcludePlaylist.Cmd>())).ReturnsAsync(Ok(Mocks.excludedPlaylist))
  ...
  member _.``should send IdCannotBeParsed on id parsing error``() =
    ...
    presetService
      ...
      .ReturnsAsync(Error(ExcludePlaylist.Error.IdParsing(Playlist.IdParsingError "bad")))
  ...
  member _.``should send NotFound when playlist not found``() =
    ...
    presetService
      .Setup(_.ExcludePlaylist(It.IsAny<ExcludePlaylist.Cmd>()))
      .ReturnsAsync(Error(ExcludePlaylist.Error.Load(Playlist.LoadError.NotFound)))
...
type ExcludeArtistMessageHandler() =
  let userRepo = Mock<ILoadUser>()
  let presetService = Mock<IExcludeArtist>()
  let authService = Mock<IInitAuth>()
  let resourceProvider = Mock<IResourceProvider>()
  let chatCtx = Mock<IBotService>()
  ...
  let handler =
    Message.excludeArtistMessageHandler userRepo.Object presetService.Object authService.Object resourceProvider.Object chatCtx.Object
  ...
  member _.``should handle reply with excluded artist and send message``() =
    ...
    chatCtx.Setup(_.SendMessage("Artist excluded message")).ReturnsAsync(Mocks.botMessageId)
  ...
  member _.``should handle /excludeartist command and send message``() =
    ...
    chatCtx.Setup(_.SendMessage(Messages.ArtistExcluded)).ReturnsAsync(Mocks.botMessageId)
...
type IncludeArtistMessageHandler() =
  ...
  member _.``should handle reply with included artist and send message``() =
    ...
    presetService.Setup(_.IncludeArtist(It.IsAny<IncludeArtist.Cmd>())).ReturnsAsync(Ok(Mocks.artist1))
  ...
  member _.``should send IdCannotBeParsed on id parsing error``() =
    ...
    presetService
      ...
      .ReturnsAsync(Error(IncludeArtist.Error.IdParsing(Artist.IdParsingError "invalid-id")))
  ...
  member _.``should send NotFound when artist not found``() =
    ...
    presetService
      .Setup(_.IncludeArtist(It.IsAny<IncludeArtist.Cmd>()))
      .ReturnsAsync(Error(IncludeArtist.Error.Load(Artist.LoadError.NotFound)))
